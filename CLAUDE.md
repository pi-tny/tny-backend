...
- Logging is Fastify's built-in pino: structured JSON with a per-request id, `debug` level in dev / `info` in prod, `logger: false` under `NODE_ENV=test` (tests stay quiet). The `Authorization` header is redacted. Unexpected errors are logged via `request.log.error` in `src/http/error-handler.ts`.
- `src/env/index.ts` validates `process.env` with Zod at startup and throws on invalid env. Import config via `import { env } from "@/env"` rather than reading `process.env` directly. Required: `JWT_SECRET`, `DATABASE_URL`; optional `PORT` (default 3000), `NODE_ENV` (`dev`|`test`|`production`), `DATABASE_PROVIDER` (`sqlite`|`postgres`), `CORS_ORIGIN` (default `*`), `WHATSAPP_NUMBER` (store number for the order wa.me link, default empty).
